...
class FPT_Model:
	def __init__(self,
		...
		if self.dataset == 'ptb':
			...
			self.x = tf.placeholder(tf.float32, shape=[None, 1, n_input])
	...
	def setup_model(self):
		with tf.variable_scope('model',reuse=tf.AUTO_REUSE) as scope:
			...
			self.W = tf.get_variable('W', [self.num_units, self.n_classes], initializer = tf.contrib.layers.xavier_initializer())
			self.b = tf.get_variable('b', [self.n_classes], initializer=tf.constant_initializer(0.0))

	def sequentialise_input(self):
		self.x_sequential = tf.unstack(self.x, axis = 1)
	...
	def create_output(self):
		if self.dataset == 'ptb':
			...
			self.lstm_output, self.state = tf.nn.static_rnn(self.cell, 
													   self.x_sequential,
													   initial_state = self.init_state,
													   dtype=tf.float32)
	...
	def traning_loss_sequential(self):
		...
		self.x_new_seq = tf.squeeze(self.x_t,[2])
		if self.dataset == 'ptb':
		...
		if self.use_lstm:
			...
			self.lstm_outputs_t_seq,self.state_t_seq=tf.nn.static_rnn(self.cell,self.input_t_seq,dtype="float32",initial_state=state)
