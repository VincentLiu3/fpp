...
if dataset == 'cycleworld':
	...
	pathname = 'results-nips-decay/results-cw/'
	learning_rate = 0.01
	fpt_lr = 0.01
	output_lr = 0.01
	...
	# plt.ylabel('L1 norm of beta')
	if exp == True:
		end_name_fpt = 'exp_{}.npy'.format(alpha)
	else:
		end_name_fpt = '.npy'
		# end_name_fpt = '_Wnorm.npy'
		# end_name_fpt = '_betanorm.npy'
	end_name_bptt = '.npy'
	if cycleworld_size == 6:
		baseline = 83*np.ones_like(x)
	elif cycleworld_size == 10:
		baseline = 90*np.ones_like(x)
	time_steps_series = []
	configs = []
	l = 1000
	if cycleworld_size == 10:
		configs = [(l,1,0),(l,3,0),(l,5,0),(l,10,0)]
		time_steps_series = [1,3,5,10]
	else:
		configs = [(l,1,0),(l,2,0),(l,6,0)]
	...
	pathname = 'results-nips-decay/results-sd/'
	data_name = 'sd_'
	...
	if exp == True:
		...
		end_name_fpt = '_loss.npy'
	end_name_bptt = '_loss.npy'
	time_steps_series = [1,3,5,10,20]
	configs = [(1000,1,0),(1000,5,0),(1000,10,0),(1000,20,0)]


colors = []
for time_steps in time_steps_series:
	if buffer == False and comp==False:
	...
	if buffer == False and comp==False:
		if dataset != 'ptb':
			plt.plot(x,bptt_mean,label = '{}-BPTT'.format(time_steps),color=color,linestyle='--')
...
for config in configs:
	...
	if dataset != 'ptb':
		plt.plot(x,fpt_mean,color=color,label = name+'FPP-Buffer-{}'.format(buffer_length,updates_per_step,state_updates_per_step))
		plot_error_bars(x,fpt,fpt.shape[0],color)
		print(name+'Random-FPP-Buffer-{}-n-{}: {},{}'.format(buffer_length,updates_per_step,np.mean(fpt_mean),np.std(fpt_mean)))
		if comp == True:
			plt.plot(x,fpt_mean_1,color=color,linestyle=':')
			plot_error_bars(x,fpt_1,fpt.shape[0],color)
			print(name+'Decay-FPP-Buffer-{}-n-{}: {},{}'.format(buffer_length,updates_per_step,np.mean(fpt_mean_1),np.std(fpt_mean)))
...
if dataset == 'stochastic_dataset':
	ax.legend(custom_lines, [name, 'FPP','1','5','10','15'])
elif dataset == 'cycleworld' and cycleworld_size==6:
	ax.legend(custom_lines, [name, 'FPP','1','2','6'])
elif dataset == 'cycleworld' and cycleworld_size==10:
	ax.legend(custom_lines, [name, 'FPP','1','3','5','10'])
# ax.legend(custom_lines, ['Decay', 'Random','1','5','10','15'])
# plt.legend()
plt.show()
# exit()
# ##Senstivity
