...
class ReplayBuffer():
    ...
    def sample_batch(self, num_batch, batch_len):
        """
        sample B blocks of T transitions where B = num_batch and T = batch_len
        ----
        output
        ob_batch: [T, B, input_size]
        state_batch: [T, B, hidden_size]
        y_batch: [T, B]
    ...
    def sample_all(self):
        ob_batch, state_tm1_batch, state_batch, y_batch, ind_batch = self.sample_successive(len(self._storage))
        y_batch = torch.unsqueeze(y_batch, dim=1)
        return ob_batch, state_tm1_batch, state_batch, y_batch, ind_batch
